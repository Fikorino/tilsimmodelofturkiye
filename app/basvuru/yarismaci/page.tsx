...
import { getContestantTerms, getSiteSettings } from "@/lib/data";
import { SiteHeader } from "@/components/site-header";
import { SiteFooter } from "@/components/site-footer";

export default async function ContestantApplicationPage() {
  const [terms, siteSettings] = await Promise.all([getContestantTerms(), getSiteSettings()]);
  ...
  return (
    <div className="min-h-screen bg-bg text-fg">
      ...
      <main className="mx-auto max-w-4xl px-4 py-16 md:px-6">
        ...
        <h1 className="mt-6 font-display text-3xl">Yarışmacı Başvuru</h1>
        <p className="mt-2 text-muted">Başvuru formunu eksiksiz doldurun ve belgelerinizi yükleyin.</p>

        <section className="mt-8 rounded-2xl border border-white/10 bg-white/5 p-6">
          <h2 className="text-lg font-semibold">Katılım Şartları</h2>
