...
export default async function HomePage() {
  ...
  const hero = homeSections
    ? {
        ...
        hotelImageUrl: homeSections.hotel_image_url ?? "",
        hotelAddress: homeSections.hotel_address ?? "",
        hotelMapUrl: homeSections.hotel_map_url ?? "",
        ...
        <section id="juri" className="mx-auto max-w-6xl px-4 py-16 md:px-6">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {(juryMembers.length
              ...
              <Card key={member.id} className="flex flex-col items-start gap-4">
                <Image
                ...
                <div className="flex flex-wrap gap-3 text-sm text-gold">
                  {member.socials?.instagram && <a href={member.socials.instagram}>Instagram</a>}
                  {member.socials?.tiktok && <a href={member.socials.tiktok}>TikTok</a>}
                  {member.socials?.linkedin && <a href={member.socials.linkedin}>LinkedIn</a>}
                  {member.socials?.website && <a href={member.socials.website}>Web</a>}
                </div>
              </Card>
            ))}
          </div>
